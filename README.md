...
git checkout master
git merge first
git push (2 раза, т.е. в каждой ветке по отдельности, если потребуется)
...
git merge origin (main,master) {Ветки origin/main и origin/master это корневые ветки без вложенной ветки}
